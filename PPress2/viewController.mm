...
- (void)viewDidLoad1 {
   // [super viewDidLoad];
    ...
    passiveColor = [NSColor colorWithDeviceRed:0.85f green:0.85f blue:0.85f alpha:1.0f];
    disableColor = [NSColor colorWithDeviceRed:0.5f green:0.5f blue:0.5f alpha:1.0f];
    ...
    while (fileName = [filesEnumerator nextObject])
    {
        ...
        else
        {
            *fileSize += attr.fileSize;
            ...
            if (![fileManager removeItemAtPath:NewDestinaion error:error])
                {
                //NSLog(@"Could not remove old files. Error:%@",error);
                //[error release];
                _Log("removeItemAtPath return error!")
                _Log([[*error localizedDescription] UTF8String])
                return NO;
                }
            }
            *error = nil;
            if (![fileManager copyItemAtPath:NewName toPath:NewDestinaion error:error])
            {
                ...
                _Log("copyItemAtPath return error!")
                _Log([[*error localizedDescription] UTF8String])
                return NO;
            }
...
-(void)StartCopying
{
    ...
    //make source paths
    char pluginSource[2048];
#ifdef _AUTOMATION_
    char automationSource[2048];
#endif//_AUTOMATION_
    ...
    {
        char str[1024];
        sprintf(str,"OSXVersionBig=%d, _BigSur=%d",OSXVersionBig,_BigSur);
        _Log(str)
        _Log(pluginSource)
    }
#endif
...
- (BOOL)fileManager:(NSFileManager *)fileManager shouldProceedAfterError:(NSError *)error copyingItemAtPath:(NSString *)srcPath toPath:(NSString *)dstPath
{
    ...
    NSDebugAlert([error localizedDescription],@"Installer",NSAlertStyleCritical);
    return NO;
...
void ParseBigVersion(char* str1){
   ...
   // if ((OSXVersionBig>=101600) && (OSXVersionBig<110200) )
    ...
    {
        char str[256];
        sprintf(str,"major=%d minor=%d patch=%d : OSXVersionBig=%d",major,minor,patch,OSXVersionBig);
        _Log(str)
    }
#endif
}
