...
-(void)ProcessUpdatingsDB
{
    ...
    if (_NeedToCopyTextures)
    {
        ...
        AddSetsInstallInfo( Path1, "29486", "Texture Set One",2);
        AddSetsInstallInfo( Path1, "59321", "Texture Set Two",2);
        AddSetsInstallInfo( Path1, "10734", "Texture Set Three",2);
        AddSetsInstallInfo( Path1, "82349", "Texture Set Four",2);
...
//        strcat(pluginSource,"CS5/");
    ...
    if (_NeedToCopyTextures)
    {
        strcpy(mwdSource,FullPath);
        strcat(mwdSource,"/Contents/Resources/MWDs/");
        strcat(mwdSource, _StoreFolder);
    ...
    if (_NeedToCopyTextures)
    {
        mwdSize = [self folderSizeRecurent:[NSString stringWithCString:mwdSource encoding:NSUTF8StringEncoding]];
        NSLog(@"MWD size = %llu",mwdSize);
        _LogInt("MWD size is %d",mwdSize)
    }//_NeedToCopyTextures
    ...
    //[MWD2
#ifdef _AUTOMATION_
    [progressIndicator setMaxValue:(double)(folderSize*1.05 + pluginSize + automationSize+mwdSize)];
#else
    [progressIndicator setMaxValue:(double)(folderSize*1.05 + pluginSize + mwdSize )];
#endif//_AUTOMATION_
   ...
   // if (![fileManager copyItemAtPath:[NSString stringWithCString:pluginSource encoding:NSUTF8StringEncoding] toPath:destinationPath error:&error])
    ...
    //[MWD2
    NSString *MWDSource = [NSString stringWithCString:mwdSource encoding:NSUTF8StringEncoding];
    //]
    ...
    dispatch_async(previewQueue,
                   ^{
                       ...
                       //[MWD2
                       if ((success)&&(_NeedToCopyTextures))
                       {
                           _Log("before MWD folderCopyRecurent")
                           if( [self folderCopyRecurent:MWDSource
                            destination:destinationPath copied:&curFileSize error:&error showProgress:YES])
                           {
                               success = YES;
                               NSLog(@"Copy MWD Success");
                           }
                           else
                           {
                               success = NO;
