...
- (void)viewDidLoad1 {
   // [super viewDidLoad];
    ...
    [self.view.window setBackgroundColor:[NSColor colorWithDeviceRed:233.0f/255 green:233.0f/255 blue:233.0f/255 alpha:1.0f]];
    
    // Do view setup here.
    activeColor = [NSColor colorWithDeviceRed:171.0f/255 green:74.0f/255 blue:0.0f alpha:1.0f];
    passiveColor = [NSColor colorWithDeviceRed:0.35f green:0.35f blue:0.35f alpha:1.0f];
    disableColor = [NSColor colorWithDeviceRed:0.65f green:0.65f blue:0.65f alpha:1.0f];
...
//        strcat(pluginSource,"CS5/");
   ...
   // if (![fileManager copyItemAtPath:[NSString stringWithCString:pluginSource encoding:NSUTF8StringEncoding] toPath:destinationPath error:&error])
    ...
    //Create and setup AlphaPluginsBridge.ini
    //NSString *DestPath = [NSString stringWithString:destinationPath];
    //DestPath = [DestPath stringByAppendingPathComponent:@"AlphaPluginsBridge.ini"];
    const char *tmpPath = (char*)[destinationPath UTF8String];
    char DestPath[1024];
    strcpy(DestPath,tmpPath);
    len = strlen(DestPath);
    if (len>0)
        if ((DestPath[len-1]!='/')&&(DestPath[len-1]!='\\'))
            strcat(DestPath,"/");
    strcat(DestPath,"AlphaPluginsBridge.ini");
    FILE *file;
    ...
    if (file)
    {
        ...
        len = strlen(PluginPath);
        if (len>0)
            if ((PluginPath[len-1]!='/')&&(PluginPath[len-1]!='\\'))
                strcat(PluginPath,"/");
        
        strcat(PluginPath,"AlphaPluginsLaunchBox.app");
        //if (_BreakFinding>1)
